...
(Max 20)""",font=('Comic Sans MS',14,'bold'),height=2,width=14,bg='#3c1c0f',fg='white').place(relx=0.5,rely=0.45,anchor=CENTER)
    entry_var = tk.StringVar(value='3')
    entry_var.trace('w', on_validate)
    entry1 = tk.Entry(playG, validate="key", validatecommand=(playG.register(on_validate), '%d', '%P'), textvariable=entry_var,font=20,justify=CENTER)
    entry1.place(relx=0.5,rely=0.5,anchor=CENTER)
    entry1.config(width=19,borderwidth=2,relief='sunken',bg="white",fg="#3c1c0f")
...
def startgame():
    ...
    if int(no_of_doors) > 20 or int(no_of_doors) <= 2:
...
def montyGame():
    ...
    num_labels=no_of_doors
    door_img = Image.open("door_img.png").resize((100, 200), Image.LANCZOS)
    door_img = ImageTk.PhotoImage(door_img)

    def create_labels(num_labels):
        dis=120
        for i in range(num_labels):
            label = tk.Label(playM, image=door_img, text=f"Door {i+1}", bg="#3c1c0f", fg="white", compound="top")
            if dis<=1400:
                label.place(x=-40+dis, y=140)
                dis+=140
            else:
                label.place(x=-40+dis-1400, y=390)
                dis+=140
    create_labels(num_labels)

    def Result():
        global playM,entry2,choosen_doors,Choose_door_button,ch_door,dum_door,Choose_door_button1,Choose_door_button2,ch_door2,goat_img,door_img2
        ...
        num_labels=no_of_doors
        goat_img = Image.open("goat_img.png").resize((100, 200), Image.LANCZOS)
        goat_img = ImageTk.PhotoImage(goat_img)
        door_img2 = Image.open("door_img.png").resize((100, 200), Image.LANCZOS)
        door_img2 = ImageTk.PhotoImage(door_img2)
        
        def create_labels(num_labels):
            global goat_img, door_img2
            
            dis=120
            if choosen_doors!=car_selected_door:
                for i in range(1,num_labels+1):
                    if i != choosen_doors and i != car_selected_door:
                        label = tk.Label(playM, image=goat_img, text=f"Door {i}", bg="#3c1c0f", fg="white", compound="top")
                    else:
                        label = tk.Label(playM, image=door_img2, text=f"Door {i}", bg="#3c1c0f", fg="white", compound="top")
                ...
                for i in range(1,num_labels+1):
                    if i != choosen_doors and i != dum_door:
                        label = tk.Label(playM, image=goat_img, text=f"Door {i}", bg="#3c1c0f", fg="white", compound="top")
                    else:
                        label = tk.Label(playM, image=door_img2, text=f"Door {i}", bg="#3c1c0f", fg="white", compound="top")
    ...
    entry_var = tk.StringVar(value='3')
    entry_var.trace('w', on_validate)
    entry2 = tk.Entry(playM, validate="key", validatecommand=(playM.register(on_validate), '%d', '%P'), textvariable=entry_var,font=20,justify=CENTER)
    entry2.place(relx=0.1,rely=0.9,anchor=CENTER)
    entry2.config(width=19,borderwidth=2,relief='sunken',bg="white",fg="#3c1c0f")
...
def swap_door():
    global choosen_doors,car_selected_door,dum_door,Choose_door_button1,Choose_door_button2,ch_door2,car_img,goat_img
    if choosen_doors==car_selected_door:
    ...
    if choosen_doors==car_selected_door:
        ...
        wb.save(ex_filename)
        rev()
        win_lab=tk.Label(playM,text=f'''You WIN !!! 
Win Door: {car_selected_door} 
Swaped Door: {choosen_doors}''',font=('Comic Sans MS',14,'bold'),height=4,width=20,bg='#3c1c0f',fg='white')
        win_lab.place(relx=0.5,rely=0.08,anchor=CENTER)
        ...
        wb.save(ex_filename)
        rev()
        lose_lab=tk.Label(playM,text=f'''You LOSE !!!
Win Door: {car_selected_door} 
Swaped Door: {choosen_doors}''',font=('Comic Sans MS',14,'bold'),height=4,width=20,bg='#3c1c0f',fg='white')
        lose_lab.place(relx=0.5,rely=0.08,anchor=CENTER)
        winsound.PlaySound('lose_Sound.wav', winsound.SND_FILENAME)
...
def stay_door():    
    global choosen_doors,car_selected_door,dum_door,Choose_door_button1,Choose_door_button2,ch_door2,car_img
    ...
    if choosen_doors==car_selected_door:
        ...
        win_lab=tk.Label(playM,text=f'''You WIN
Win Door: {car_selected_door} 
Stayed Door: {choosen_doors}''',font=('Comic Sans MS',14,'bold'),height=4,width=20,bg='#3c1c0f',fg='white')
        win_lab.place(relx=0.5,rely=0.08,anchor=CENTER)
        winsound.PlaySound('win_sound.wav', winsound.SND_FILENAME)
...
def rev():
    global car_img,goat_img
    num_labels=no_of_doors
    goat_img = Image.open("goat_img.png").resize((100, 200), Image.LANCZOS)
    goat_img = ImageTk.PhotoImage(goat_img)
    car_img = Image.open("car_img.png").resize((100, 200), Image.LANCZOS)
    ...
    def create_labels(num_labels):
        ...
        dis=120
        for i in range(1,num_labels+1):
            if i != car_selected_door:
                label = tk.Label(playM, image=goat_img, text=f"Door {i}", bg="#3c1c0f", fg="white", compound="top")
            else:
                label = tk.Label(playM, image=car_img, text=f"Door {i}", bg="#3c1c0f", fg="white", compound="top")
            if dis<=1400:
                label.place(x=-40+dis, y=140)
                dis+=140
            else:
